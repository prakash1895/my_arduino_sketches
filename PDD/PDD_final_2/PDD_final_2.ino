...
void loop()                  
{
  ...
  if (GPS_battery_value>737)  
      digitalWrite(GPS_battery_LED, HIGH);
  
  else if(GPS_battery_value <= 737)
  {
      digitalWrite(GPS_battery_LED, HIGH);
      delay(100);
  ...
  if (GSM_battery_value>737)  
      digitalWrite(GSM_battery_LED, HIGH);
  
  else if(GSM_battery_value <= 737)
  {
      digitalWrite(GSM_battery_LED, HIGH);
      delay(100);
  ...
  if ((digitalRead(Distress_button) == LOW) && (flag_distress==0))
    {
        flag_distress = 1;
        String stri;
        char str1[10],str2[10];
        DateTime now = rtc.now();
        stri= "Emergency Latitude : " + String(dtostrf( GPS.latitudeDegrees,3,6,str1)) + "   Longitude : "+String(dtostrf(GPS.longitudeDegrees,3,6,str2)) +"         "+ String(now.day()) +"/" +String(now.month()) +"/" +String(now.year()) + "          "+String(now.hour()) + ":"+ String(now.minute()) +":" +String(now.second());
        SendMessage(stri); 
    }

  else if (digitalRead(Distress_button)==HIGH)
  {
    flag_distress = 0;
  }
